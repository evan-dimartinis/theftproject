...
L.tileLayer(
  "https://api.mapbox.com/styles/v1/{id}/tiles/{z}/{x}/{y}?access_token={accessToken}",
  {
    ...
    zoomOffset: -1,
    accessToken:
      "pk.eyJ1IjoiZXZhbmRpbWFyIiwiYSI6ImNrOGl1MG1ydzAybHYzZnA4dmpnZm8zeDcifQ.piEThe--C2dKlShdSIdk4g",
  }
).addTo(mymap);
...
function onAutoStolen() {
  if (autoToggle === 1) {
    auto_stolen.forEach((element) => {
      var automarker = L.marker(element).addTo(mymap);
    });
    autoToggle === 0;
  }
  if (autoToggle === 0) {
    auto_stolen.forEach((element) => {
      var automarker = L.marker(element).remove(mymap);
    });
    autoToggle === 1;
  }
}
